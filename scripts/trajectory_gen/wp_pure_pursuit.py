...
class PurePursuit(object):
    ...
    def __init__(self):
        ...
        for i in range(datsize):
            self.traj_[i] = (float(point_data[i][0]), float(point_data[i][1]))
    ...
    def driver(self, angle, velocity):
        ...
        drive_msg.drive.speed = self.Getvelocity(error)#velocity
    ...
    def pose_callback(self, pose_msg):
        ...
        if len(closeIndex) > 1:
            if self.kdt_occ != None:
                xg, yg = self.helper(closeIndex, self.traj_)
                if self.collision_checker(xg, yg):
                    closeIndex = sorted(self.rightkdtree.query_ball_point((x, y), self.horizon))
                    xg, yg = self.helper(closeIndex, self.right_traj)
                    if self.collision_checker(xg, yg):
                        #print("right collision", xg, yg)
                        closeIndex = sorted(self.leftkdtree.query_ball_point((x, y), self.horizon))
                        xg, yg= self.helper(closeIndex, self.left_traj)
                        if self.collision_checker(xg, yg):
