...
def running_mean(x, window):
    """x is array of points and window is window size"""
...
def filter_small_triangles(points, tri, coeff):
    ...
    Inputs:
        ...
        coeff: triangles with an edge > coeff * median(edge) will be filtered
    Outputs:
...
def get_edges(sims, tolerance, pts):
    ...
    mids = get_mids(edges_array, pts)
    return edges_array, mids
...
def smoothener(pts):
    """smoothen the trajectory using spline interpolation"""
    mids = np.array(pts)
    x = mids[:, 0].flatten()
    y = mids[:, 1].flatten()
...
def tf_points(x, y):
    """
    transform points to the correct frame of reference.
    look out for the role of resolution here 0.5 or 0.05
    """
    rmx = (x*0.05 - 12.2)
    rmy = (y*0.05 - 12.2)
    return rmx, rmy
...
def get_mean_traj(ox, oy, total_points):
    """
    smoothen the trajectory using running mean filter.
    Calculates running mean and closes the loop.
    """
    rmx = running_mean(ox, total_points)
    rmy = running_mean(oy, total_points)
    # for i in range(int(total_points/2)-1):
    #     rmx = np.append(rmx, ox[-(int(total_points/2) - i - 1 )])
    #     rmy = np.append(rmy, oy[-(int(total_points/2) - i - 1)])
...
def publish_plan(xs, ys, publisher):
    """Publishes the plan to the topic /trajectory"""
    msg = Path()
...
def pad_with(vector, pad_width, iaxis, kwargs):
    """Pad the vector with the given pad value."""
...
def map_inflate(f):
    """Inflate the map and return the inflated map"""
    size = 6
