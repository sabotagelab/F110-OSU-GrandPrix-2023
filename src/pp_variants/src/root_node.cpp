...
public:
  ...
  RootNode()
  : odom_sub_(), scan_sub_(), marker_pub_(),
  ...
  {
    // returns distance between given trajectory and
    //  its nearest occupied point
    float r = 1.0f;
  ...
  void scanCallback(const sensor_msgs::LaserScan::ConstPtr & scan_in)
  {
    ...
    int i = 0;
    std::string target_frame = "map";
    laser_to_map = tf_buffer.lookupTransform(
      target_frame, scan_in->header.frame_id, ros::Time(
        0), ros::Duration(1.0) );
    map_points.clear();
    for (int i = 0; i < cloud.points.size(); ++i) {
      if (i > min_angle && i < max_angle && i % 3 == 0) {
        ...
        p.pose.position.x = static_cast<float>(cloud.points[i].x);
        p.pose.position.y = static_cast<float>(cloud.points[i].y);
  ...
  void publish_enums()
  {
    ...
    float r = 1.0f;
    std_msgs::Int8 k;
    // add a single line to get nearest map point to trajectory
    float min_dist_to_center = min_dist_to_traj(refwp);
