...
RRT::RRT(ros::NodeHandle &nh): nh_(nh), gen((std::random_device())()) {
    ...
    ORIGIN.position.x = 0;
    ORIGIN.position.y = 0; //int(-GRID_WIDTH/(2));
...
std::vector<double> RRT::sample() {
    ...
    // TODO:: FIX THIS FUNCTION generating points out of the range far away
    // Returns:
    ...
    if (occ_grid.data[int(y + rmax*x)] > 20)
        {sample();}
    sampled_point.push_back(x);
    sampled_point.push_back(y);
    //std::cout<<x<<"  and "<< rmax<< " y is "<<y<<std::endl;
    //std::cout<<"end stample"<<std::endl;
...
int RRT::nearest(std::vector<Node> &tree, std::vector<double> &sampled_point) {
    ...
    //std::cout<<"start nearest"<<std::endl;
    int min = 10000;
    int nearest_node = 0;
    ...
    //std::cout<<"end nearest  "<<nearest_node <<std::endl;
    return nearest_node;
}
