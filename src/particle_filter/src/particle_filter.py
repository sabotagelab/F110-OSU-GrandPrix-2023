...
class ParticleFiler:
    """This class implements Monte Carlo Localization based on odometry."""
    ...
    def __init__(self):
        ...
        self.pose_pub = rospy.Publisher(
            rospy.get_param("~pf_pose_topic", "/pf/viz/inferred_pose"),
            PoseStamped,
            queue_size=1,
        )
        self.particle_pub = rospy.Publisher(
            rospy.get_param("~particles_viz_topic", "/pf/viz/particles"),
            PoseArray,
            queue_size=1,
        )
        self.pub_fake_scan = rospy.Publisher(
            rospy.get_param("~fake_scan_viz_topic", "/pf/viz/fake_scan"),
            LaserScan,
            queue_size=1,
        )
        self.rect_pub = rospy.Publisher(
            rospy.get_param("~poly_viz_topic", "/pf/viz/poly1"),
            PolygonStamped,
            queue_size=1,
        )
        ...
        if self.PUBLISH_ODOM:
            self.odom_pub = rospy.Publisher(
                rospy.get_param("~pf_odometry_pub_topic", "/pf/pose/odom"),
                Odometry,
                queue_size=1,
            )

        # these topics are for coordinate space things
        self.base_link_name = rospy.get_param("~base_link_name", "/car1_30m/base_link")
        ...
        self.click_sub = rospy.Subscriber(
            rospy.get_param("~clicked_point_topic", "/clicked_point"),
            PointStamped,
            self.clicked_pose,
            queue_size=1,
        )
    ...
    def get_omap(self):
        """
        Fetch the occupancy grid map from the map_server instance.

        Initialize the correct RangeLibc method.
        Also stores a matrix which indicates the permissible region of the map
        """
    ...
    def publish_tf(self, pose, stamp=None):
        ...
        map_laser_pos -= np.dot(
            tf.transformations.quaternion_matrix(
                tf.transformations.unit_vector(map_laser_rotation)
            )[:3, :3],
            laser_base_link_offset,
        ).T

        # Publish transform
        self.pub_tf.sendTransform(
            map_laser_pos, map_laser_rotation, stamp, self.base_link_name, "/map"
        )
    ...
    def lidarCB(self, msg):
        """Initialize reused buffers, and stores the relevant laser scanner data for later use."""
    ...
    def precompute_sensor_model(self):
        ...
        For each discrete computed range value, this provides the
        probability of measuring any (discrete) range.

        This table is indexed by the sensor model at runtime by
        discretizing the measurements and computed ranges from RangeLibc.
        """
    ...
    def motion_model(self, proposal_dist, action):
        """
        Apply the odometry to the particle distribution.

        If the odometry data is inaccurate, the motion model mixes
        in gaussian noise to spread out the distribution.
    ...
    def sensor_model(self, proposal_dist, obs, weights):
        """
        Compute a probablistic weight for each particle in the proposal distribution.
    ...
    def MCL(self, a, o):
        """
        Perform one step of Monte Carlo Localization.
...
parser.add_argument(
    "--config",
    help="Path to yaml file containing config parameters. Helpful for calling node directly with Python for profiling.",
)
